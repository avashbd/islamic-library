...
export default function BulkAdd() {
  const { addBook, addCategory, categories } = useLibrary();
  ...
  async function handleSaveAll() {
    setSaving(true);
    const usableRows = rows.filter((r) => r.status === "done" && r.title.trim());
    for (const r of usableRows) {
      if (r.category && !categories.includes(r.category)) {
        addCategory(r.category);
      }
      addBook({
        title: r.title.trim(),
      ...
      {rows.length > 0 && (
        <>
          ...
          <button
            ...
            disabled={saving || fetchingAll || doneCount === 0}
          >
            {saving ? "সংরক্ষণ হচ্ছে…" : `সব বই সংরক্ষণ করুন (${doneCount}টি)`}
          </button>
        </>
      )}
    </div>
